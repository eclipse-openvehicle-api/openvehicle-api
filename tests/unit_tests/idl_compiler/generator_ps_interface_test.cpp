#include "includes.h"
#include "generated/ps/interfaces_proxy.cpp"
#include "generated/ps/interfaces_stub.cpp"
#include <interfaces/ps/core_proxy.cpp>
#include <interfaces/ps/core_stub.cpp>
...
public:
    ...
    */
    ...
    virtual void ProcessMegaStruct(/*inout*/ SMegaStruct& rsStruct) override
    {
        ...
        //rsStruct.sUnnamedVal.i += 20;
        // Anonymous structs are not supported (since C++ doesn't support them either)
        //rsStruct.iAnonymous1 += 10;
        //rsStruct.iAnonymous2 += 20;
        // Anonymous and unnamed unions are supported if not at global scope.
        rsStruct.uUnnamedValA.switch_to(0);
        rsStruct.uUnnamedValA.iCaseA0 = 10;
        EXPECT_EQ(rsStruct.uUnnamedValA.iCaseA1, rsStruct.uUnnamedValA.iCaseA0);
        rsStruct.switch_to(0);
        rsStruct.uUnnamedValC.iCaseC0 = 20;
        EXPECT_EQ(rsStruct.uUnnamedValC.iCaseC1, rsStruct.uUnnamedValC.iCaseC0);
        rsStruct.iCaseD0 = 30;
        EXPECT_EQ(rsStruct.iCaseD1, rsStruct.iCaseD0);
        ...
        rsStruct.rgnVal[0]++;
        rsStruct.rgnVal[1] += 2;
        rsStruct.rgeHelloVal[0]++;
        rsStruct.rgeHelloVal[1]--;
    ...
    */
    virtual void ProcessMegaTypeBasedUnion(/*inout*/ UMegaTypeBasedUnion& ruUnion) override
    {
        ...
        case 24: ruUnion.eHelloVal++;           break;
        case 25:
        ...
        case 224: ruUnion.teHelloVal++;             break;
        case 225:
    ...
    */
    virtual void ProcessMegaVarBasedUnion(/*inout*/ SMegaVarBasedUnion& rsUnion) override
    {
        ...
        case 24: rsUnion.uVal.eHelloVal++;           break;
        case 25:
        ...
        case 224: rsUnion.uVal.teHelloVal++;           break;
        case 225:
...
using CGeneratorInterfaceTest = CGeneratorTest;

TEST_F(CGeneratorInterfaceTest, MarshallClientToServer)
{
    // Test object pretending to be client and server
    CInterfaceTest test;

    // Create a stub object as if the repository would do this
    ISayHello__stub stub;

    // The CGeneratorInterfaceTest test class pretends to be the component isolation manager
    RegisterStubObject(sdv::interface_t(static_cast<ISayHello*>(&test)), &stub);

    // Create the proxy object
    ISayHello__proxy proxy;

    // The CGeneratorInterfaceTest test class pretends to be the component isolation manager
    RegisterProxyObject(sdv::interface_t(static_cast<ISayHello*>(&test)), &stub, &proxy);

    // Get the target interface
    ISayHello* pSayHello = proxy.GetTargetInterface().template get<ISayHello>();

    ASSERT_NE(pSayHello, nullptr);
    EXPECT_EQ(pSayHello->Hello(), "Hello");
}

TEST_F(CGeneratorInterfaceTest, MarshallInterfaceFromServer)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
    ...
    // Create a stub object as if the repository would do this
    ISayHello__stub stub;
    IRequestHello__stub stubRequest;

    // The test class pretends to be the component isolation manager
    RegisterStubObject(sdv::interface_t(static_cast<ISayHello*>(&test)), &stub);
    RegisterStubObject(sdv::interface_t(static_cast<IRequestHello*>(&test)), &stubRequest);

    // Create the proxy object
    ISayHello__proxy proxy;
    IRequestHello__proxy proxyRequest;

    // The CGeneratorInterfaceTest test class pretends to be the component isolation manager
    RegisterProxyObject(sdv::interface_t(static_cast<ISayHello*>(&test)), &stub, &proxy);
    RegisterProxyObject(sdv::interface_t(static_cast<IRequestHello*>(&test)), &stubRequest, &proxyRequest);

    // Call Request on IRequestHello directly (GetInterface is not registered here and will fail when marshalling)
    // This will access the ISayHello__stub-object.
    ISayHello* pSayHello = proxyRequest.Access().Request();
...
TEST_F(CGeneratorInterfaceTest, MarshallInterfaceFromClient)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
    ...
    // Create a stub object as if the repository would do this
    ISayHello__stub stub;
    IRegisterHelloCallback__stub stubRegister;

    // The test class pretends to be the component isolation manager
    RegisterStubObject(sdv::interface_t(static_cast<ISayHello*>(&test)), &stub);
    RegisterStubObject(sdv::interface_t(static_cast<IRegisterHelloCallback*>(&test)), &stubRegister);

    // Create the proxy object
    ISayHello__proxy proxy;
    IRegisterHelloCallback__proxy proxyRegister;

    // The CGeneratorInterfaceTest test class pretends to be the component isolation manager
    RegisterProxyObject(sdv::interface_t(static_cast<ISayHello*>(&test)), &stub, &proxy);
    RegisterProxyObject(sdv::interface_t(static_cast<IRegisterHelloCallback*>(&test)), &stubRegister, &proxyRegister);

    // Call Register on IRegisterHelloCallback directly (GetInterface is not registered here and will fail when marshalling)
    // This will link a stub object on the client side to the server proxy.
    proxyRegister.Access().Register(&test);
...
TEST_F(CGeneratorInterfaceTest, MarshallGenericInterfaceFromServer)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
    ...
    ISayHello__stub stub;
    IRequestHello__stub stubRequest;
    sdv__IInterfaceAccess__stub stubInterfaceAccess;

    // The test class pretends to be the component isolation manager
    RegisterStubObject(sdv::interface_t(static_cast<ISayHello*>(&test)), &stub);
    RegisterStubObject(sdv::interface_t(static_cast<IRequestHello*>(&test)), &stubRequest);
    RegisterStubObject(sdv::interface_t(static_cast<sdv::IInterfaceAccess*>(&test)), &stubInterfaceAccess);

    // Create the proxy object
    ISayHello__proxy proxy;
    IRequestHello__proxy proxyRequest;
    sdv__IInterfaceAccess__proxy proxyInterfaceAccess;

    // The CGeneratorInterfaceTest test class pretends to be the component isolation manager
    RegisterProxyObject(sdv::interface_t(static_cast<ISayHello*>(&test)), &stub, &proxy);
    RegisterProxyObject(sdv::interface_t(static_cast<IRequestHello*>(&test)), &stubRequest, &proxyRequest);
    RegisterProxyObject(sdv::interface_t(static_cast<sdv::IInterfaceAccess*>(&test)), &stubInterfaceAccess, &proxyInterfaceAccess);

    // Call GetInterface to get the IInterfaceAccess pointer once more (to be able to test marshalling).
    sdv::IInterfaceAccess* pInterfaceAccess = proxyInterfaceAccess.Access().GetInterface<sdv::IInterfaceAccess>();
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaStructBoolean)
{
    // Test object pretending to be client and server
    CInterfaceTest test;

    // Create a stub object as if the repository would do this
    IMegaTest__stub stub;

    // The CGeneratorInterfaceTest test class pretends to be the component isolation manager
    RegisterStubObject(sdv::interface_t(static_cast<IMegaTest*>(&test)), &stub);

    // Create the proxy object
    IMegaTest__proxy proxy;

    // The CGeneratorInterfaceTest test class pretends to be the component isolation manager
    RegisterProxyObject(sdv::interface_t(static_cast<IMegaTest*>(&test)), &stub, &proxy);

    // Get the target interface
    IMegaTest* pMegaTest = proxy.GetTargetInterface().template get<IMegaTest>();
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaStructIntegral)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaStructCharacter)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaStructFloatingPoint)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaStructEnum)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaStructString)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaStructPointer)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaStructSequence)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaStructInterface)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
    ...
    // Create a stub object as if the repository would do this
    IMegaTest__stub stub;
    IMultiplyValue__stub stubMultiplyMember;
    IAddValue__stub stubAddMember;
    IMultiplyValue__stub stubMultiplyLocal;
    ...
    // The CGeneratorInterfaceTest test class pretends to be the component isolation manager
    RegisterStubObject(sdv::interface_t(static_cast<IMegaTest*>(&test)), &stub);
    RegisterStubObject(sdv::interface_t(static_cast<IMultiplyValue*>(&test.m_multiplier)), &stubMultiplyMember);
    RegisterStubObject(sdv::interface_t(static_cast<IAddValue*>(&test.m_summer)), &stubAddMember);
    RegisterStubObject(sdv::interface_t(static_cast<IMultiplyValue*>(&sMultiply)), &stubMultiplyLocal);

    // Create the proxy object
    IMegaTest__proxy proxy;
    IMultiplyValue__proxy proxyMultiplyMember;
    IAddValue__proxy proxyAddMember;
    IMultiplyValue__proxy proxyMultiplyLocal;

    // The CGeneratorInterfaceTest test class pretends to be the component isolation manager
    RegisterProxyObject(sdv::interface_t(static_cast<IMegaTest*>(&test)), &stub, &proxy);
    RegisterProxyObject(sdv::interface_t(static_cast<IMultiplyValue*>(&test.m_multiplier)), &stubMultiplyMember, &proxyMultiplyMember);
    RegisterProxyObject(sdv::interface_t(static_cast<IAddValue*>(&test.m_summer)), &stubAddMember, &proxyAddMember);
    RegisterProxyObject(sdv::interface_t(static_cast<IMultiplyValue*>(&sMultiply)), &stubMultiplyLocal, &proxyMultiplyLocal);
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaStructSubCompound)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaStructBooleanArray)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
    ...
    EXPECT_FALSE(sStruct.rgbVal[0]);
    EXPECT_TRUE(sStruct.rgbVal[1]);
    EXPECT_NO_THROW(pMegaTest->ProcessMegaStruct(sStruct));
    EXPECT_TRUE(sStruct.rgbVal[0]);
    EXPECT_FALSE(sStruct.rgbVal[1]);
}

TEST_F(CGeneratorInterfaceTest, MarshallMegaStructIntegralArray)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaStructCharacterArray)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaStructFloatingPointArray)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaStructEnumArray)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaStructStringArray)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaStructPointerArray)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaStructSequenceArray)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
// Incompatible serdes generation: BUG #398509
TEST_F(CGeneratorInterfaceTest, DISABLED_MarshallMegaStructInterfaceArray)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaStructSubCompoundArray)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaStructBooleanTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaStructIntegralTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaStructCharacterTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaStructFloatingPointTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaStructEnumTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaStructStringTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
// Using typedefs of template based types produced an error: BUG #398266
TEST_F(CGeneratorInterfaceTest, DISABLED_MarshallMegaStructPointerTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
// Using typedefs of template based types produced an error: BUG #398266
TEST_F(CGeneratorInterfaceTest, DISABLED_MarshallMegaStructSequenceTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaStructInterfaceTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
    ...
    // The CGeneratorInterfaceTest test class pretends to be the component isolation manager
    RegisterStubObject(sdv::interface_t(static_cast<IMegaTest*>(&test)), &stub);
    RegisterStubObject(sdv::interface_t(static_cast<IMultiplyValue*>(&test.m_multiplierTypedef)), &stubMultiplyMember);
    RegisterStubObject(sdv::interface_t(static_cast<IAddValue*>(&test.m_summerTypedef)), &stubAddMember);
    RegisterStubObject(sdv::interface_t(static_cast<IMultiplyValue*>(&sMultiply)), &stubMultiplyLocal);
    ...
    // The CGeneratorInterfaceTest test class pretends to be the component isolation manager
    RegisterProxyObject(sdv::interface_t(static_cast<IMegaTest*>(&test)), &stub, &proxy);
    RegisterProxyObject(sdv::interface_t(static_cast<IMultiplyValue*>(&test.m_multiplierTypedef)), &stubMultiplyMember, &proxyMultiplyMember);
    RegisterProxyObject(sdv::interface_t(static_cast<IAddValue*>(&test.m_summerTypedef)), &stubAddMember, &proxyAddMember);
    RegisterProxyObject(sdv::interface_t(static_cast<IMultiplyValue*>(&sMultiply)), &stubMultiplyLocal, &proxyMultiplyLocal);
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaStructSubCompoundTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaTypeBasedUnionBoolean)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaTypeBasedUnionIntegral)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaTypeBasedUnionCharacter)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaTypeBasedUnionFloatingPoint)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaTypeBasedUnionEnum)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaTypeBasedUnionString)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaTypeBasedUnionPointer)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaTypeBasedUnionSequence)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaTypeBasedUnionInterface)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaTypeBasedUnionSubCompound)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaTypeBasedUnionBooleanTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaTypeBasedUnionIntegralTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaTypeBasedUnionCharacterTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaTypeBasedUnionFloatingPointTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaTypeBasedUnionEnumTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaTypeBasedUnionStringTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
// Using typedefs of template based types produced an error: BUG #398266
TEST_F(CGeneratorInterfaceTest, DISABLED_MarshallMegaTypeBasedUnionPointerTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
// Using typedefs of template based types produced an error: BUG #398266
TEST_F(CGeneratorInterfaceTest, DISABLED_MarshallMegaTypeBasedUnionSequenceTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaTypeBasedUnionInterfaceTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaTypeBasedUnionSubCompoundTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaVarBasedUnionBoolean)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaVarBasedUnionIntegral)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaVarBasedUnionCharacter)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaVarBasedUnionFloatingPoint)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaVarBasedUnionEnum)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaVarBasedUnionString)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaVarBasedUnionPointer)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaVarBasedUnionSequence)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaVarBasedUnionInterface)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaVarBasedUnionSubCompound)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaVarBasedUnionBooleanTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaVarBasedUnionIntegralTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaVarBasedUnionCharacterTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaVarBasedUnionFloatingPointTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaVarBasedUnionEnumTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaVarBasedUnionStringTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
// Using typedefs of template based types produced an error: BUG #398266
TEST_F(CGeneratorInterfaceTest, DISABLED_MarshallMegaVarBasedUnionPointerTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
// Using typedefs of template based types produced an error: BUG #398266
TEST_F(CGeneratorInterfaceTest, DISABLED_MarshallMegaVarBasedUnionSequenceTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaVarBasedUnionInterfaceTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
...
TEST_F(CGeneratorInterfaceTest, MarshallMegaVarBasedUnionSubCompoundTypedef)
{
    // Test object pretending to be client and server
    CInterfaceTest test;
