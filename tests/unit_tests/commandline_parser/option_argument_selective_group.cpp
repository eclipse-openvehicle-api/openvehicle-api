...
TEST_F(CCommandLineParserTestArgumentSelection, OptionParseBoolean)
{
    CCommandLine cl;
    bool bSelect = false;
    auto& rSelect = cl.DefineOption("select", bSelect, "Select it!", true, 0, 1, 2);
...
TEST_F(CCommandLineParserTestArgumentSelection, OptionParseIntegralIndependent)
{
    CCommandLine cl;
    int8_t i8Var = 0;
...
TEST_F(CCommandLineParserTestArgumentSelection, OptionParseIntegralDedependent)
{
    CCommandLine cl;
    signed char cVar = 0;
    auto& rCVar = cl.DefineOption("cvar", cVar, "char variable", true, 0, 1);
    short sVar = 0;
    auto& rSVar = cl.DefineOption("svar", sVar, "short variable", true, 1, 2);
    long lVar = 0;
    auto& rLVar = cl.DefineOption("lvar", lVar, "long variable", true, 2, 3);
    int iVar = 0;
    auto& rIVar = cl.DefineOption("ivar", iVar, "int variable", true, 3, 4);
    long long llVar = 0;
    auto& rLLVar = cl.DefineOption("llvar", llVar, "long long variable", true, 4, 5);
    unsigned char ucVar = 0;
    auto& rUCVar = cl.DefineOption("ucvar", ucVar, "unsigned char variable", true, 5, 6);
    unsigned short usVar = 0;
    auto& rUSVar = cl.DefineOption("usvar", usVar, "unsigned short variable", true, 6, 7);
    unsigned long ulVar = 0;
    auto& rULVar = cl.DefineOption("ulvar", ulVar, "unsigned long variable", true, 7, 8);
    unsigned int uiVar = 0;
    auto& rUIVar = cl.DefineOption("uivar", uiVar, "unsigned int variable", true, 8, 9);
    unsigned long long ullVar = 0;
    auto& rULLVar = cl.DefineOption("ullvar", ullVar, "unsigned long long variable", true, 9, 10);
    size_t nVar = 0;
    auto& rNVar = cl.DefineOption("nvar", nVar, "size_t variable", true, 10, 11);
...
TEST_F(CCommandLineParserTestArgumentSelection, OptionParseIntegralIndependentVector)
{
    // Attention: std::vector<int8_t> causes compiler problems with MSVC and GCC.
    CCommandLine cl;
    //std::vector<int8_t> veci8Var;
    //auto& rI8Var = cl.DefineOption("i8var", veci8Var, "vector of int8_t variable", true, 0, 1);
    std::vector<int16_t> veci16Var;
    auto& rI16Var = cl.DefineOption("i16var", veci16Var, "vector of int16_t variable", true, 1, 2);
    std::vector<int32_t> veci32Var;
    auto& rI32Var = cl.DefineOption("i32var", veci32Var, "vector of int32_t variable", true, 2, 3);
    std::vector<int64_t> veci64Var;
    auto& rI64Var = cl.DefineOption("i64var", veci64Var, "vector of int64_t variable", true, 3, 4);
    std::vector<uint8_t> vecui8Var;
    auto& rUI8Var = cl.DefineOption("ui8var", vecui8Var, "vector of uint8_t variable", true, 4, 5);
    std::vector<uint16_t> vecui16Var;
    auto& rUI16Var = cl.DefineOption("ui16var", vecui16Var, "vector of uint16_t variable", true, 5, 6);
    std::vector<uint32_t> vecui32Var;
    auto& rUI32Var = cl.DefineOption("ui32var", vecui32Var, "vector of uint32_t variable", true, 6, 7);
    std::vector<uint64_t> vecui64Var;
    auto& rUI64Var = cl.DefineOption("ui64var", vecui64Var, "vector of uint64_t variable", true, 7, 8);
...
TEST_F(CCommandLineParserTestArgumentSelection, OptionParseIntegralDedependentVector)
{
    CCommandLine cl;
    std::vector<signed char> veccVar;
    auto& rCVar = cl.DefineOption("cvar", veccVar, "vector of char variable", true, 0, 1);
    std::vector<short> vecsVar;
    auto& rSVar = cl.DefineOption("svar", vecsVar, "vector of short variable", true, 1, 2);
    std::vector<long> veclVar;
    auto& rLVar = cl.DefineOption("lvar", veclVar, "vector of long variable", true, 2, 3);
    std::vector<int> veciVar;
    auto& rIVar = cl.DefineOption("ivar", veciVar, "vector of int variable", true, 3, 4);
    std::vector<long long> vecllVar;
    auto& rLLVar = cl.DefineOption("llvar", vecllVar, "vector of long long variable", true, 4, 5);
    std::vector<unsigned char> vecucVar;
    auto& rUCVar = cl.DefineOption("ucvar", vecucVar, "vector of unsigned char variable", true, 5, 6);
    std::vector<unsigned short> vecusVar;
    auto& rUSVar = cl.DefineOption("usvar", vecusVar, "vector of unsigned short variable", true, 6, 7);
    std::vector<unsigned long> veculVar;
    auto& rULVar = cl.DefineOption("ulvar", veculVar, "vector of unsigned long variable", true, 7, 8);
    std::vector<unsigned int> vecuiVar;
    auto& rUIVar = cl.DefineOption("uivar", vecuiVar, "vector of unsigned int variable", true, 8, 9);
    std::vector<unsigned long long> vecullVar;
    auto& rULLVar = cl.DefineOption("ullvar", vecullVar, "vector of unsigned long long variable", true, 9, 10);
    std::vector<size_t> vecnVar;
    auto& rNVar = cl.DefineOption("nvar", vecnVar, "list of size_t variable", true, 10, 11);
...
TEST_F(CCommandLineParserTestArgumentSelection, OptionParseIntegralIndependentSequence)
{
    // Attention: sdv::sequence<int8_t> causes compiler problems with MSVC and GCC.
    CCommandLine cl;
    //sdv::sequence<int8_t> seqi8Var;
    //auto& rI8Var = cl.DefineOption("i8var", seqi8Var, "sequence of int8_t variable", true, 0, 1);
    sdv::sequence<int16_t> seqi16Var;
    auto& rI16Var = cl.DefineOption("i16var", seqi16Var, "sequence of int16_t variable", true, 1, 2);
    sdv::sequence<int32_t> seqi32Var;
    auto& rI32Var = cl.DefineOption("i32var", seqi32Var, "sequence of int32_t variable", true, 2, 3);
    sdv::sequence<int64_t> seqi64Var;
    auto& rI64Var = cl.DefineOption("i64var", seqi64Var, "sequence of int64_t variable", true, 3, 4);
    sdv::sequence<uint8_t> sequi8Var;
    auto& rUI8Var = cl.DefineOption("ui8var", sequi8Var, "sequence of uint8_t variable", true, 4, 5);
    sdv::sequence<uint16_t> sequi16Var;
    auto& rUI16Var = cl.DefineOption("ui16var", sequi16Var, "sequence of uint16_t variable", true, 5, 6);
    sdv::sequence<uint32_t> sequi32Var;
    auto& rUI32Var = cl.DefineOption("ui32var", sequi32Var, "sequence of uint32_t variable", true, 6, 7);
    sdv::sequence<uint64_t> sequi64Var;
    auto& rUI64Var = cl.DefineOption("ui64var", sequi64Var, "sequence of uint64_t variable", true, 7, 8);
...
TEST_F(CCommandLineParserTestArgumentSelection, OptionParseIntegralDedependentSequence)
{
    CCommandLine cl;
    sdv::sequence<signed char> seqcVar;
    auto& rCVar = cl.DefineOption("cvar", seqcVar, "sequence of char variable", true, 0, 1);
    sdv::sequence<short> seqsVar;
    auto& rSVar = cl.DefineOption("svar", seqsVar, "sequence of short variable", true, 1, 2);
    sdv::sequence<long> seqlVar;
    auto& rLVar = cl.DefineOption("lvar", seqlVar, "sequence of long variable", true, 2, 3);
    sdv::sequence<int> seqiVar;
    auto& rIVar = cl.DefineOption("ivar", seqiVar, "sequence of int variable", true, 3, 4);
    sdv::sequence<long long> seqllVar;
    auto& rLLVar = cl.DefineOption("llvar", seqllVar, "sequence of long long variable", true, 4, 5);
    sdv::sequence<unsigned char> sequcVar;
    auto& rUCVar = cl.DefineOption("ucvar", sequcVar, "sequence of unsigned char variable", true, 5, 6);
    sdv::sequence<unsigned short> sequsVar;
    auto& rUSVar = cl.DefineOption("usvar", sequsVar, "sequence of unsigned short variable", true, 6, 7);
    sdv::sequence<unsigned long> sequlVar;
    auto& rULVar = cl.DefineOption("ulvar", sequlVar, "sequence of unsigned long variable", true, 7, 8);
    sdv::sequence<unsigned int> sequiVar;
    auto& rUIVar = cl.DefineOption("uivar", sequiVar, "sequence of unsigned int variable", true, 8, 9);
    sdv::sequence<unsigned long long> sequllVar;
    auto& rULLVar = cl.DefineOption("ullvar", sequllVar, "sequence of unsigned long long variable", true, 9, 10);
    sdv::sequence<size_t> seqnVar;
    auto& rNVar = cl.DefineOption("nvar", seqnVar, "list of size_t variable", true, 10, 11);
...
TEST_F(CCommandLineParserTestArgumentSelection, OptionParseIntegralIndependentList)
{
    // Attention: std::list<int8_t> causes compiler problems with MSVC and GCC.
    CCommandLine cl;
    //std::list<int8_t> lsti8Var;
    //auto& rI8Var = cl.DefineOption("i8var", lsti8Var, "list of int8_t variable", true, 0, 1);
    std::list<int16_t> lsti16Var;
    auto& rI16Var = cl.DefineOption("i16var", lsti16Var, "list of int16_t variable", true, 1, 2);
    std::list<int32_t> lsti32Var;
    auto& rI32Var = cl.DefineOption("i32var", lsti32Var, "list of int32_t variable", true, 2, 3);
    std::list<int64_t> lsti64Var;
    auto& rI64Var = cl.DefineOption("i64var", lsti64Var, "list of int64_t variable", true, 3, 4);
    std::list<uint8_t> lstui8Var;
    auto& rUI8Var = cl.DefineOption("ui8var", lstui8Var, "list of uint8_t variable", true, 4, 5);
    std::list<uint16_t> lstui16Var;
    auto& rUI16Var = cl.DefineOption("ui16var", lstui16Var, "list of uint16_t variable", true, 5, 6);
    std::list<uint32_t> lstui32Var;
    auto& rUI32Var = cl.DefineOption("ui32var", lstui32Var, "list of uint32_t variable", true, 6, 7);
    std::list<uint64_t> lstui64Var;
    auto& rUI64Var = cl.DefineOption("ui64var", lstui64Var, "list of uint64_t variable", true, 7, 8);
...
TEST_F(CCommandLineParserTestArgumentSelection, OptionParseIntegralDedependentList)
{
    CCommandLine cl;
    std::list<signed char> lstcVar;
    auto& rCVar = cl.DefineOption("cvar", lstcVar, "list of char variable", true, 0, 1);
    std::list<short> lstsVar;
    auto& rSVar = cl.DefineOption("svar", lstsVar, "list of short variable", true, 1, 2);
    std::list<long> lstlVar;
    auto& rLVar = cl.DefineOption("lvar", lstlVar, "list of long variable", true, 2, 3);
    std::list<int> lstiVar;
    auto& rIVar = cl.DefineOption("ivar", lstiVar, "list of int variable", true, 3, 4);
    std::list<long long> lstllVar;
    auto& rLLVar = cl.DefineOption("llvar", lstllVar, "list of long long variable", true, 4, 5);
    std::list<unsigned char> lstucVar;
    auto& rUCVar = cl.DefineOption("ucvar", lstucVar, "list of unsigned char variable", true, 5, 6);
    std::list<unsigned short> lstusVar;
    auto& rUSVar = cl.DefineOption("usvar", lstusVar, "list of unsigned short variable", true, 6, 7);
    std::list<unsigned long> lstulVar;
    auto& rULVar = cl.DefineOption("ulvar", lstulVar, "list of unsigned long variable", true, 7, 8);
    std::list<unsigned int> lstuiVar;
    auto& rUIVar = cl.DefineOption("uivar", lstuiVar, "list of unsigned int variable", true, 8, 9);
    std::list<unsigned long long> lstullVar;
    auto& rULLVar = cl.DefineOption("ullvar", lstullVar, "list of unsigned long long variable", true, 9, 10);
    std::list<size_t> lstnVar;
    auto& rNVar = cl.DefineOption("nvar", lstnVar, "list of size_t variable", true, 10, 11);
...
TEST_F(CCommandLineParserTestArgumentSelection, OptionParseFloatingPoint)
{
    CCommandLine cl;
    float fVar = 0;
    auto& rFVar = cl.DefineOption("fvar", fVar, "float variabl1e", true, 0, 1);
    double dVar = 0;
    auto& rDVar = cl.DefineOption("dvar", dVar, "double variable", true, 1, 2);
    long double ldVar = 0;
    auto& rLDVar = cl.DefineOption("ldvar", ldVar, "long double variable", true, 2, 3);
...
TEST_F(CCommandLineParserTestArgumentSelection, OptionParseFloatingPointVector)
{
    CCommandLine cl;
    std::vector<float> vecfVar;
    auto& rFVar = cl.DefineOption("fvar", vecfVar, "vector of float variable", true, 0, 1);
    std::vector<double> vecdVar;
    auto& rDVar = cl.DefineOption("dvar", vecdVar, "vector of double variable", true, 1, 2);
    std::vector<long double> vecldVar;
    auto& rLDVar = cl.DefineOption("ldvar", vecldVar, "vector of long double variable", true, 2, 3);
...
TEST_F(CCommandLineParserTestArgumentSelection, OptionParseFloatingPointSequence)
{
    CCommandLine cl;
    sdv::sequence<float> seqfVar;
    auto& rFVar = cl.DefineOption("fvar", seqfVar, "sequence of float variable", true, 0, 1);
    sdv::sequence<double> seqdVar;
    auto& rDVar = cl.DefineOption("dvar", seqdVar, "sequence of double variable", true, 1, 2);
    sdv::sequence<long double> seqldVar;
    auto& rLDVar = cl.DefineOption("ldvar", seqldVar, "sequence of long double variable", true, 2, 3);
...
TEST_F(CCommandLineParserTestArgumentSelection, OptionParseFloatingPointList)
{
    CCommandLine cl;
    std::list<float> lstfVar;
    auto& rFVar = cl.DefineOption("fvar", lstfVar, "list of float variable", true, 0, 1);
    std::list<double> lstdVar;
    auto& rDVar = cl.DefineOption("dvar", lstdVar, "list of double variable", true, 1, 2);
    std::list<long double> lstldVar;
    auto& rLDVar = cl.DefineOption("ldvar", lstldVar, "list of long double variable", true, 2, 3);
...
TEST_F(CCommandLineParserTestArgumentSelection, OptionParseEnum)
{
    ...
    auto& rEUVar = cl.DefineOption("unscoped_enum", eUnscopedTest, "unscoped enum variable", true, 0, 1);
    rEUVar.AddAssociations(rgsUnscopedEnumAssociations);
    EScopedTest eScopedTest = EScopedTest::test4;
    auto& rESVar = cl.DefineOption("scoped_enum", eScopedTest, "scoped enum variable", true, 1, 2);
...
TEST_F(CCommandLineParserTestArgumentSelection, OptionParseEnumVector)
{
    enum EUnscopedTest { test1, test2, test3 };
    enum class EScopedTest : size_t {test4, test5, test6};
    CCommandLine cl;
    ...
    auto& rEUVar = cl.DefineOption("unscoped_enum", vecUnscopedTest, "unscoped enum variable", true, 0, 1);
    rEUVar.AddAssociations(rgsUnscopedEnumAssociations);
    std::vector<EScopedTest> vecScopedTest;
    auto& rESVar = cl.DefineOption("scoped_enum", vecScopedTest, "scoped enum variable", true, 1, 2);
    rESVar.AddAssociations(rgsScopedEnumAssociations);
...
TEST_F(CCommandLineParserTestArgumentSelection, OptionParseEnumSequence)
{
    enum EUnscopedTest { test1, test2, test3 };
    enum class EScopedTest : size_t {test4, test5, test6};
    CCommandLine cl;
    ...
    auto& rEUVar = cl.DefineOption("unscoped_enum", seqUnscopedTest, "unscoped enum variable", true, 0, 1);
    rEUVar.AddAssociations(rgsUnscopedEnumAssociations);
    sdv::sequence<EScopedTest> seqScopedTest;
    auto& rESVar = cl.DefineOption("scoped_enum", seqScopedTest, "scoped enum variable", true, 1, 2);
    rESVar.AddAssociations(rgsScopedEnumAssociations);
...
TEST_F(CCommandLineParserTestArgumentSelection, OptionParseEnumList)
{
    enum EUnscopedTest { test1, test2, test3 };
    enum class EScopedTest : size_t {test4, test5, test6};
    CCommandLine cl;
    ...
    auto& rEUVar = cl.DefineOption("unscoped_enum", lstUnscopedTest, "unscoped enum variable", true, 0, 1);
    rEUVar.AddAssociations(rgsUnscopedEnumAssociations);
    std::list<EScopedTest> lstScopedTest;
    auto& rESVar = cl.DefineOption("scoped_enum", lstScopedTest, "scoped enum variable", true, 1, 2);
    rESVar.AddAssociations(rgsScopedEnumAssociations);
...
TEST_F(CCommandLineParserTestArgumentSelection, OptionParseString)
{
    CCommandLine cl;
    std::string ssVar1;
    auto& rSSVar = cl.DefineOption("ssvar1", ssVar1, "std::string variable", true, 0, 1);
    sdv::u8string ssu8Var1;
    auto& rSSU8Var = cl.DefineOption("ssu8var1", ssu8Var1, "sdv::u8string variable", true, 1, 2);
...
TEST_F(CCommandLineParserTestArgumentSelection, OptionParseStringVector)
{
    CCommandLine cl;
    std::vector<std::string> vecssVar1;
    auto& rSSVar = cl.DefineOption("ssvar1", vecssVar1, "std::vector<std::string> variable", true, 0, 1);
    std::vector<sdv::u8string> vecssu8Var1;
    auto& rSSU8Var = cl.DefineOption("ssu8var1", vecssu8Var1, "std::vector<sdv::u8string> variable", true, 1, 2);
...
TEST_F(CCommandLineParserTestArgumentSelection, OptionParseStringSequence)
{
    CCommandLine cl;
    sdv::sequence<std::string> seqssVar1;
    auto& rSSVar = cl.DefineOption("ssvar1", seqssVar1, "sdv::sequence<std::string> variable", true, 0, 1);
    sdv::sequence<sdv::u8string> seqssu8Var1;
    auto& rSSU8Var = cl.DefineOption("ssu8var1", seqssu8Var1, "sdv::sequence<sdv::u8string> variable", true, 1, 2);
...
TEST_F(CCommandLineParserTestArgumentSelection, OptionParseStringList)
{
    CCommandLine cl;
    std::list<std::string> lstssVar1;
    auto& rSSVar = cl.DefineOption("ssvar1", lstssVar1, "std::list<std::string> variable", true, 0, 1);
    std::list<sdv::u8string> lstssu8Var1;
    auto& rSSU8Var = cl.DefineOption("ssu8var1", lstssu8Var1, "std::list<sdv::u8string> variable", true, 1, 2);
...
TEST_F(CCommandLineParserTestArgumentSelection, OptionParsePath)
{
    CCommandLine cl;
    std::filesystem::path pathVar1;
    auto& rPATHVar = cl.DefineOption("pathvar1", pathVar1, "std::filesystem::path variable", true, 0, 1);
...
TEST_F(CCommandLineParserTestArgumentSelection, OptionParsePathVector)
{
    CCommandLine cl;
    std::vector<std::filesystem::path> vecpathVar1;
    auto& rPATHVar = cl.DefineOption("pathvar1", vecpathVar1, "std::vector<std::filesystem::path> variable", true, 0, 1);
...
TEST_F(CCommandLineParserTestArgumentSelection, OptionParsePathSequence)
{
    CCommandLine cl;
    sdv::sequence<std::filesystem::path> seqpathVar1;
    auto& rPATHVar = cl.DefineOption("pathvar1", seqpathVar1, "sdv::sequence<std::filesystem::path> variable", true, 0, 1);
...
TEST_F(CCommandLineParserTestArgumentSelection, OptionParsePathList)
{
    CCommandLine cl;
    std::list<std::filesystem::path> lstpathVar1;
    auto& rPATHVar = cl.DefineOption("pathvar1", lstpathVar1, "std::list<std::filesystem::path> variable", true, 0, 1);
...
TEST_F(CCommandLineParserTestArgumentSelection, OptionParseFlags)
{
    CCommandLine cl;
    bool bFlag = false;
    auto& rFLAGVar = cl.DefineFlagOption("flag", bFlag, "flag variable", true, 0, 1);
